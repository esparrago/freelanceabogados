jQuery(document).ready(function($) {
	...
	$(document).on( 'click', '.upload_button', function( event ) {
		...
		file_frame.on( 'open', function() {
			...
			// hide labels for the search input and the close icon
			$('.search-form .screen-reader-text').hide();
			$('.media-modal-close .screen-reader-text').hide();
		...
		function display_custom_notes() {
			...
			// check if notes were already displayed
			if ( undefined != $('.app-media-settings').html() ) {
				return;
			}
