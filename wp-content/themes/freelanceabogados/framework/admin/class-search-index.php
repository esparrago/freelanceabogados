...
class APP_Search_Index_Admin extends APP_Conditional_Tabs_Page {
		...
		if ( appthemes_get_search_index_status() ) {
			...
			$fields = array(
				array(
					...
					'desc' => __( 'The Search Index is now completely built.', APP_TD ),
				),
				array(
					'title' => __( 'Delete Search Index', APP_TD ),
					'type' => 'submit',
					'name' => array( 'search_index', 'delete_index' ),
					'extra' => array(
						'class' => 'button-secondary',
						'onclick' => 'return confirm("' . __( 'WARNING: You are about to completely delete search index. Are you sure you want to proceed?', APP_TD ) . '");',
					),
					'value' => __( 'Delete Search Index', APP_TD ),
					'desc' =>
						'<br />' . __( 'Do you wish to completely delete search index?', APP_TD ) .
						'<br />' . __( 'Once you do this you will be able to recreate it.', APP_TD ) .
						'<br />' . __( 'Use it only if you made some major changes to what should be indexed.', APP_TD ),
				),
			);
			...
			$fields = array(
				array(
					...
					'desc' => html( 'div', array( 'id' => 'search-index-message' ), __( 'The Search Index is now being build...', APP_TD ) ) . $this->progress_bar(),
				),
			);
			...
			$fields = array(
				array(
					'title' => __( 'Updating speed', APP_TD ),
					'type' => 'radio',
					'name' => array( 'search_index', 'speed' ),
					'values' => array(
						'regular' => __( 'Regular', APP_TD ),
						'faster' => __( 'Faster', APP_TD ),
						'fast' => __( 'Fast', APP_TD ),
						'fastest' => __( 'Fastest', APP_TD ),
					),
					'desc' => '',
					'tip' => __( 'This let you choose speed of updating index. If you are not on dedicated server use the Regular option.', APP_TD ),
				),
				array(
					'title' => __( 'Update Search Index', APP_TD ),
					'type' => 'submit',
					'name' => array( 'search_index', 'update_index' ),
					'extra' => array(
						'class' => 'button-secondary',
					),
					'value' => __( 'Update Search Index', APP_TD ),
					'desc' => '',
				),
			);
	...
	public static function get_index_speed( $speed = 'regular' ) {
	...
	public function ajax_build_index() {
		...
		if ( appthemes_get_search_index_status() ) {
			die( json_encode( array( 'success' => true, 'completed' => true, 'message' => __( 'The Search Index is now completely built.', APP_TD ) ) ) );
		}
		...
		die( json_encode( array( 'success' => true, 'completed' => false, 'message' => __( 'Building Search Index has run.', APP_TD ) ) ) );
	}
...
<?php
		if ( ! empty( $_POST['search_index']['update_index'] ) ) {
			...
			$speed = isset( $_POST['search_index']['speed'] ) ? $_POST['search_index']['speed'] : 'regular';
